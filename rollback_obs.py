    ...
    for need_version_folder in os.listdir(OLD_ELBA):
        ...
        if cnt == change:
            for item in os.listdir(OLD_ELBA + need_version_folder):
                if item.endswith('.py'):
                    os.system(f"{get_peculiarities_system('copy_file')}{OLD_ELBA}{need_version_folder}/{item} .")
            template_remove_folder(FOLDER_WITH_DATA)
            os.system(
                f"{get_peculiarities_system('copy_dir')}{OLD_ELBA}{need_version_folder}/{FOLDER_WITH_DATA}/ ."
            )
    system_action('clear')
